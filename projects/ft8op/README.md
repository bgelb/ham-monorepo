...
`ft8op` is the receive, control, and web UI application for this repo. It combines:
...
By default, when run from the workspace root, `ft8op` loads [config/ft8op.json](config/ft8op.json) from this project.
...
The FSM lives in [ft8op/src/qso.rs](projects/ft8op/src/qso.rs).
...
`ft8op` supports the WSJT-X DXpedition compound message:
